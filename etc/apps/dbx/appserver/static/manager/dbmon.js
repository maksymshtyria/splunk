$(function(){
    ...
    var inputs = { database: $('#item-database select'), table: $('#item-table input'), name: $('#item-name input'), type: $('#item-type select') };
    var tablesLoading = {}, columnsLoading = {};

    if(!/\/_new$/.test($('form#eaiform').attr("action"))) {
        inputs.database.enable(false);
        inputs.type.enable(false);
    }

    function fetchTables(db) {
        if(tablesLoading[db]) return;
        tablesLoading[db] = true;
        $.getJSON(Splunk.util.make_url("/custom/dbx/dbinfo/database_tables"), { database: db, nocache: String(new Date().getTime()) }, function(data) {
        ...
        if(v) {
            if(autoCompleteTables) {
                if(v != autoCompleteTables.db) {
                    try { inputs.table.autocomplete("destroy"); } catch(e){ /* noop */ }
    ...
    var nameLastSet;
    function updateName() {
        var val = inputs.table.val();
        if(val) {
            var curVal = inputs.name.val();
            if((!curVal) || (curVal == nameLastSet)) {
                nameLastSet = val;
                inputs.name.val(val);
            }
        }
